...
//
	...
	for(int i=0; i< U.nrows*U.ncols; i++){
		U.raw[i].r = unif_distribution(generator);
		U.raw[i].i =/*0.;*/ unif_distribution(generator);
	}
	//V
	for(int i=0; i< nrowsV*ncolsV; i++){
		V.raw[i].r = unif_distribution(generator);
		V.raw[i].i = unif_distribution(generator);
	}
...
//
    ...
    int pippo;
    fileSVIn.read((char *)&pippo,sizeof(integer));
    cout << "\nS matrix dimensions: "<< numSV << " " << pippo << "\n";
...
//
	for(int i=0; i<ncolsS; i++){
		//__DEBUG
		printf(" %f\n ",appo[i]);
		S.raw[i*S.nrows +i].r =appo[i];
		S.raw[i*S.nrows +i].i =0.;
	}
////
